...
//Expression simplifies to ltr with index. Index may be frac, which will be in simplest form.
    ...
    do {
        ...
        xd = p1d * r1 * p2d * r2 * p3d * r3;
            ...
            (p1d * r1) === (p2d * r2) || Math.abs(p1n) === Math.abs(r1 * p1d) || Math.abs(p2n) === Math.abs(r2 * p2d) || Math.abs(p3n) === Math.abs(r3 * p3d) || 
            Math.abs(p1n / (r1 * p1d)) === Math.abs(p3n / (r3 * p3d)) || Math.abs(p2n / (r2 * p2d)) === Math.abs(p3n / (r3 * p3d)))
                    //Sets difficulty level (max numerator & max denominator), avoids same numerator & denominator or either being 0,
                    //avoids term1 & term2 denoms being same and any term numerator equalling its denominator.
                    //Also avoides term 1 or term 2 indexes equalling term 3 index (easy cancel)
