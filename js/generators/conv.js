...
export function generate() {
  ...
  let m, v, merr, verr, mmax, vmax, ke, keerr;

  sumarrconv = utils.QLimitRepeats(sumarrconv, 7);
  const sum = sumarrconv[sumarrconv.length - 1];

  switch (sum) {
    case 1:     //Absolute & relative error - Kinetic Energy
      notesLink = "images/20200504-MathsBook6ErrConvv1_3-APO.pdf#page=3";
      m = utils.rndgen(10, 30, 0, 1, -1);
      v = utils.rndgen(2, 8, 0, 1, -1);
      merr = utils.rndgen(2, 8, 0, 1, -1);
      verr = utils.rndgen(2, 10, 0, 1, -1);
      mmax = utils.dp(m * (1 + merr / 100), 3, -1);
      vmax = utils.dp(v * (1 + verr / 100), 3, -1);
      ke = utils.dp(0.5 * m * Math.pow(v, 2), 3, -1);
      keerr = utils.dp(0.5 * (m * (1 + merr / 100)) * Math.pow(v * (1 + verr / 100), 2), 3, -1);
      ...
      suma += "Absolute\\ error&=Actual-Expected=" + keerr + "-" + ke + "=\\underline{\\mathbf{" + 
              utils.dp(keerr-ke, 2, 2) + "\\ J\\ (2\\ dp)}}\\\\[10pt]";
      suma += "Relative\\ error&=\\frac{Actual-Expected}{Expected}\\times100=\\frac{" + 
              keerr + "-" + ke + "}{" + ke + "}\\times100=\\underline{\\mathbf{" + 
              utils.dp(((keerr-ke)/ke)*100, 2, 2) + "\\ \\%\\ (2\\ dp)}}\\end{aligned}$$";
      break;
    case 2: //Absolute & relative error - Power
      notesLink = "images/20200504-MathsBook6ErrConvv1_3-APO.pdf#page=3";
      var I = utils.rndgen(2, 13, 0, 1, -1);
      var R = utils.rndgen(50, 100, 0, 5, -1);
      var Ierr = utils.rndgen(2, 8, 0, 1, -1);
      var Rerr = utils.rndgen(3, 12, 0, 1, -1);
      var Imax = utils.dp(I * (1 + Ierr / 100), 3, -1);
      var Rmax = utils.dp(R * (1 + Rerr / 100), 3, -1);
      var P = utils.dp(Math.pow(I, 2) * R, 3, -1);
      var Perr = utils.dp(Math.pow(Imax, 2) * Rmax, 3, -1);
      ...
      suma += "Max\\ possible\\ P&=I^2R=" + Imax + "^2\\times" + Rmax + "=" + utils.thouSep(Perr, "\\ ") + "\\ W\\\\[25pt]";
      suma += "Absolute\\ error&=Actual-Expected=" + utils.thouSep(Perr, "\\ ") + "-" + utils.thouSep(P, "\\ ") + "=\\underline{\\mathbf{" + utils.dp(Perr - P, 2, 2) + "\\ W\\ (2\\ dp)}}\\\\[10pt]";
      suma += "Relative\\ error&=\\frac{Actual-Expected}{Expected}\\times100=\\frac{" + utils.thouSep(Perr, "\\ ") + "-" + utils.thouSep(P, "\\ ") + "}{" + utils.thouSep(P, "\\ ") + "}\\times100=\\underline{\\mathbf{" + 
                  utils.dp(((Perr-P)/P)*100, 2, 2) + "\\ \\%\\ (2\\ dp)}}";
      suma += "\\end{aligned}$$";
      break;
    ...
    case 6:
    case 7:
      let qdata = [   //Units convert from, units convert to, min qty, max qty, conversion factor, rd role in calc
      ...
      notesLink = "images/20200504-MathsBook6ErrConvv1_3-APO.pdf#page=9";
      let sel = utils.rndgen(0, 15, 0, 1, -1);      //Selects row of qdata to use for question
      let qty = utils.rndgen(qdata[sel][2], qdata[sel][3], 0, 1, -1);
      let rd = utils.rndgen(0.78, 0.82, 2, 0.01, -1);
      sumq += "Convert " + utils.thouSep(qty, " ") + " " + qdata[sel][0] + 
      ...
      if(qdata[sel][5] === "mult") {      //qty to mass
          ...
          suma += utils.thouSep(qty, "\\ ") + "\\times" + qdata[sel][4] + "\\times" + rd + "&=\\\\[5pt]";
          suma += "\\underline{\\mathbf{" + utils.thouSep(utils.dp(qty * qdata[sel][4] * rd, 1, 1), "\\ ") + "\\ " + 
                  qdata[sel][1] + "\\ (1\\ utils.dp)}}\\end{aligned}$$";
      } else if(qdata[sel][5] === "divide") {     //mass to qty
          suma += "$$\\begin{aligned}quantity\\times conversion\\ factor\\div relative\\ density&=\\\\[5pt]";
          suma += utils.thouSep(qty, "\\ ") + "\\times" + qdata[sel][4] + "\\div" + rd + "&=\\\\[5pt]";
          suma += "\\underline{\\mathbf{" + utils.thouSep(utils.dp(qty * qdata[sel][4] / rd, 1, 1), "\\ ") + "\\ " + 
          ...
          suma += utils.thouSep(qty, "\\ ") + "\\times" + qdata[sel][4] + "&=\\\\[5pt]";
          suma += "\\underline{\\mathbf{" + utils.thouSep(utils.dp(qty * qdata[sel][4], 1, 1), "\\ ") + "\\ " + 
                  qdata[sel][1] + "\\ (1\\ dp)}}\\end{aligned}$$";
      }
      break;

    default:
      sumq = "Errors & Conversions question (case " + sum + ")";
