...
// ---------- original helpers (minimal change) ----------
...
// Original scaleDraw – kept virtually unchanged
function scaleDraw(ctx, xpositive, ypositive, xscale, yscale, ltr1txt, ltr2txt, solX, solY) {
  ...
  // Light intermediate lines – original
  ctx.lineWidth = 0.15;
  for (let i = 0; i < 5; i++) {
    for (let j = 1; j < 5; j++) {
      if (xpositive) {
        ctx.moveTo(xscaleposn[i] - 10 * j, xposn);
        ctx.lineTo(xscaleposn[i] - 10 * j, yscaleposn[5]);
      } else {
        ctx.moveTo(xscaleposn[i] + 10 * j, xposn);
        ctx.lineTo(xscaleposn[i] + 10 * j, yscaleposn[5]);
      }
      if (ypositive) {
        ctx.moveTo(yposn, yscaleposn[i] - 10 * j);
        ctx.lineTo(xscaleposn[5], yscaleposn[i] - 10 * j);
      } else {
        ctx.moveTo(yposn, yscaleposn[i] + 10 * j);
        ctx.lineTo(xscaleposn[5], yscaleposn[i] + 10 * j);
      }
...
export function generate() {
  ...
  // Generation (original constraints)
  do {
    do {
  ...
  return {
    ...
    canvas: {
      ...
      draw: (ctx) => {
        ...
        // Original endpoint selection (the part that made the lines reach the edges)
        // We reuse the same variables the original used
        let xcross1 = c1 / xcf1;
        let xcross2 = c2 / xcf2;
        let ycross1 = c1 / ycf1;
        let ycross2 = c2 / ycf2;
        ...
        // Draw the lines using the original endpoints
